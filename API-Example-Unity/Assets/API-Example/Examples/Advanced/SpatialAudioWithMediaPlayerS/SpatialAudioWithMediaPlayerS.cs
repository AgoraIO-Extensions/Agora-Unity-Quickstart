...
namespace Agora_RTC_Plugin.API_Example.Examples.Advanced.SpatialAudioWithMediaPlayerS
{
    public class SpatialAudioWithMediaPlayerS : MonoBehaviour
    {
        ...
        public string UidUseInEx = "123";
        public string UidUseInMPK = "67890";

        public ILocalSpatialAudioEngineS SpatialAudioEngine;
        public int x = 0;
        ...
        private void JoinChannelEx(string channelName, string uid)
        {
            ...
            RtcConnectionS connection = new RtcConnectionS();
            connection.channelId = channelName;
            connection.localUserAccount = uid;
        ...
        private void JoinChannelExWithMPK(string channelName, string uid, int playerId)
        {
            RtcConnectionS connection = new RtcConnectionS();
            connection.channelId = channelName;
            connection.localUserAccount = uid;
        ...
        private void onLeftLocationPress()
        {
            ...
            var ret = SpatialAudioEngine.UpdateRemotePositionEx(UidUseInMPK, remoteVoicePositionInfo, new RtcConnectionS(_channelName, UidUseInEx));
        ...
        internal static void MakeVideoView(string userAccount, string channelId = "", VIDEO_SOURCE_TYPE source = VIDEO_SOURCE_TYPE.VIDEO_SOURCE_REMOTE)
        {
    ...
    {
        private readonly SpatialAudioWithMediaPlayerS _spatialAudio;

        internal MpkEventHandler(SpatialAudioWithMediaPlayerS spatialAudio)
        {
        ...
        public override void OnPlayerSourceStateChanged(MEDIA_PLAYER_STATE state, MEDIA_PLAYER_ERROR ec)
        {
            ...
            if (state == MEDIA_PLAYER_STATE.PLAYER_STATE_OPEN_COMPLETED)
            {
                ...
                SpatialAudioWithMediaPlayerS.MakeVideoView(_spatialAudio.UidUseInMPK, _spatialAudio.GetChannelName());
            }
        }
    }
    ...
    internal class UserEventHandlerS : IRtcEngineEventHandlerS
    {
        ...
        internal UserEventHandlerS(SpatialAudioWithMediaPlayerS spatialAudio)
        {
            _spatialAudio = spatialAudio;
        }
        ...
        public override void OnLeaveChannel(RtcConnectionS connection, RtcStats stats)
        {
            ...
            SpatialAudioWithMediaPlayerS.DestroyVideoView("");
        }
        ...
        {
            _spatialAudio.Log.UpdateLog(string.Format("OnUserJoined uid: ${0} elapsed: ${1}", userAccount, elapsed));
            if (userAccount == _spatialAudio.UidUseInMPK)
            {
                _spatialAudio.Log.UpdateLog(string.Format("OnUserJoined uid: ${0} elapsed: ${1}", userAccount, elapsed));

            }
            else if (userAccount == _spatialAudio.UidUseInEx)
            {
                _spatialAudio.Log.UpdateLog(string.Format("OnUserJoined uid: ${0} elapsed: ${1}", userAccount, elapsed));
                SpatialAudioWithMediaPlayerS.MakeVideoView(userAccount, _spatialAudio.GetChannelName());
            }
        }

        public override void OnUserOffline(RtcConnectionS connection, string userAccount, USER_OFFLINE_REASON_TYPE reason)
        {
            _spatialAudio.Log.UpdateLog(string.Format("OnUserOffLine uid: ${0}, reason: ${1}", userAccount,
                (int)reason));
            SpatialAudioWithMediaPlayerS.DestroyVideoView("");
        }
    }
