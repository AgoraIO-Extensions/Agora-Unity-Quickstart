...
namespace Agora_RTC_Plugin.API_Example.Examples.Advanced.AudioMixingS
{
    public class AudioMixingS : MonoBehaviour
    {
    ...
    {
        private readonly AudioMixingS _audioMixing;

        internal UserEventHandlerS(AudioMixingS audioMixing)
        {
        ...
        {
            _audioMixing.Log.UpdateLog(string.Format("OnUserJoined userAccount: ${0} elapsed: ${1}", userAccount, elapsed));
        }

        public override void OnUserOffline(RtcConnectionS connection, string userAccount, USER_OFFLINE_REASON_TYPE reason)
        {
            _audioMixing.Log.UpdateLog(string.Format("OnUserOffLine userAccount: ${0}, reason: ${1}", userAccount,
                (int)reason));
        }
