...
namespace Agora_RTC_Plugin.API_Example.Examples.Advanced.AudioSpectrumS
{
    public class AudioSpectrumS : MonoBehaviour
    {
        ...
        private void InitMediaPlayer()
        {
            ...
            MpkEventHandlerS handler = new MpkEventHandlerS(this);
            MediaPlayer.InitEventHandler(handler);
            this.Log.UpdateLog("playerId id: " + MediaPlayer.GetId());

            MediaPlayer.RegisterMediaPlayerAudioSpectrumObserver(new UserAudioSpectrumObserverS(this), 16);
        }
    ...
    internal class MpkEventHandlerS : IMediaPlayerSourceObserver
    {
        private readonly AudioSpectrumS _sample;

        internal MpkEventHandlerS(AudioSpectrumS sample)
        {
            _sample = sample;
        }
        ...
        public override void OnPlayerSourceStateChanged(MEDIA_PLAYER_STATE state, MEDIA_PLAYER_ERROR ec)
        {
            ...
            {
                AudioSpectrumS.MakeVideoView(_sample.MediaPlayer.GetId().ToString(), "", VIDEO_SOURCE_TYPE.VIDEO_SOURCE_MEDIA_PLAYER);
            ...
            {
                AudioSpectrumS.DestroyVideoView(_sample.MediaPlayer.GetId().ToString());
    ...
    internal class UserEventHandlerS : IRtcEngineEventHandlerS
    {
        ...
        internal UserEventHandlerS(AudioSpectrumS sample)
        {
            _sample = sample;
        }
        ...
        public override void OnLeaveChannel(RtcConnectionS connection, RtcStats stats)
        {
            ...
            AudioSpectrumS.DestroyVideoView("");
        }
        ...
        public override void OnUserOffline(RtcConnectionS connection, string userAccount, USER_OFFLINE_REASON_TYPE reason)
        {
            _sample.Log.UpdateLog(string.Format("OnUserOffLine userAccount: ${0}, reason: ${1}", userAccount,
                (int)reason));
            AudioSpectrumS.DestroyVideoView(userAccount);
        }
    }
    ...
    internal class UserAudioSpectrumObserverS : IAudioSpectrumObserver
    {
        AudioSpectrumS _sample;
     
        internal UserAudioSpectrumObserverS(AudioSpectrumS sample)
        {
            this._sample = sample;
        }
