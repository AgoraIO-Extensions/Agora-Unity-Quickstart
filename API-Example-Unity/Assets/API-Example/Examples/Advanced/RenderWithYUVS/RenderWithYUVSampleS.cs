...
namespace Agora_RTC_Plugin.API_Example.Examples.Basic.RenderWithYUVSampleS
{
    public class RenderWithYUVSampleS : MonoBehaviour
    {
        ...
        internal void MakeVideoView(string userAccount, string channelId = "", bool useYUV = false, bool usePlane = false)
        {
            ...
            if (usePlane)
                videoSurface = MakePlaneSurface(userAccount, useYUV);
            else
                videoSurface = MakeImageSurface(userAccount, useYUV);
        ...
        private VideoSurfaceS MakePlaneSurface(string goName, bool useYUV = false)
        {
            ...
            VideoSurfaceS videoSurface = null;
            if (useYUV)
                videoSurface = go.AddComponent<VideoSurfaceYUVS>();
            else
                videoSurface = go.AddComponent<VideoSurfaceS>();

            return videoSurface;
        }

        // Video TYPE 2: RawImage
        private VideoSurfaceS MakeImageSurface(string goName, bool useYUV = false)
        {
    ...
    {
        private readonly RenderWithYUVSampleS _sample;

        private HashSet<string> usersInChannel = new HashSet<string>();

        internal UserEventHandlerS(RenderWithYUVSampleS videoSample)
        {
            _sample = videoSample;
        }
        ...
        public override void OnJoinChannelSuccess(RtcConnectionS connection, int elapsed)
        {
            ...
            _sample.MakeVideoView("", "", _sample.YUVToggle.isOn, _sample.PlaneToggle.isOn);
        }
        ...
        public override void OnLeaveChannel(RtcConnectionS connection, RtcStats stats)
        {
            ...
            RenderWithYUVSampleS.DestroyVideoView("");
            foreach (var userAccount in this.usersInChannel)
            {
                RenderWithYUVSampleS.DestroyVideoView(userAccount);
            }
        ...
        public override void OnUserJoined(RtcConnectionS connection, string userAccount, int elapsed)
        {
            ...
            _sample.MakeVideoView(userAccount, _sample.GetChannelName(), _sample.YUVToggle.isOn, _sample.PlaneToggle.isOn);
            this.usersInChannel.Add(userAccount);
        }
        ...
        public override void OnUserOffline(RtcConnectionS connection, string userAccount, USER_OFFLINE_REASON_TYPE reason)
        {
            _sample.Log.UpdateLog(string.Format("OnUserOffLine uid: ${0}, reason: ${1}", userAccount,
                (int)reason));
            RenderWithYUVSampleS.DestroyVideoView(userAccount);
            this.usersInChannel.Remove(userAccount);
        }
    }
