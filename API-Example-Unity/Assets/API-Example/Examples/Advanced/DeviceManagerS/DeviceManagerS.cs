...
namespace Agora_RTC_Plugin.API_Example.Examples.Advanced.DeviceManagerS
{
    public class DeviceManagerS : MonoBehaviour
    {
    ...
    {
        private readonly DeviceManagerS _deviceManagerSample;

        internal UserEventHandlerS(DeviceManagerS deviceManagerSample)
        {
        ...
        {
            _deviceManagerSample.Log.UpdateLog(string.Format("OnUserJoined userAccount: ${0} elapsed: ${1}", userAccount, elapsed));
        }

        public override void OnUserOffline(RtcConnectionS connection, string userAccount, USER_OFFLINE_REASON_TYPE reason)
        {
            _deviceManagerSample.Log.UpdateLog(string.Format("OnUserOffLine uid: ${0}, reason: ${1}", userAccount,
                (int)reason));
        }
    }
