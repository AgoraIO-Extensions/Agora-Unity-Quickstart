...
namespace Agora_RTC_Plugin.API_Example.Examples.Advanced.ScreenShareWhileVideoCallS
{
    public class ScreenShareWhileVideoCallS : MonoBehaviour
    {
...
#endif
            options.clientRoleType.SetValue(CLIENT_ROLE_TYPE.CLIENT_ROLE_BROADCASTER);
            var ret = RtcEngine.JoinChannelEx(_token, new RtcConnectionS(_channelName, this.Uid2), options);
    ...
    {
        private readonly ScreenShareWhileVideoCallS _desktopScreenShare;

        internal UserEventHandlerS(ScreenShareWhileVideoCallS desktopScreenShare)
        {
        ...
        public override void OnJoinChannelSuccess(RtcConnectionS connection, int elapsed)
        {
            ...
            if (connection.localUserAccount == _desktopScreenShare.Uid1)
            {
                ScreenShareWhileVideoCallS.MakeVideoView("");
            }
            else if (connection.localUserAccount == _desktopScreenShare.Uid2)
            {
                ScreenShareWhileVideoCallS.MakeVideoView("", "", VIDEO_SOURCE_TYPE.VIDEO_SOURCE_SCREEN);
            }
        }
        ...
        public override void OnLeaveChannel(RtcConnectionS connection, RtcStats stats)
        {
            ...
            if (connection.localUserAccount == _desktopScreenShare.Uid1)
            {
                ScreenShareWhileVideoCallS.DestroyVideoView("MainCameraView");
            }
            else if (connection.localUserAccount == _desktopScreenShare.Uid2)
            {
                ScreenShareWhileVideoCallS.DestroyVideoView("ScreenShareView");
            }
        }
        ...
        {
            _desktopScreenShare.Log.UpdateLog(string.Format("OnUserJoined userAccount: ${0} elapsed: ${1}", userAccount, elapsed));
            if (userAccount != _desktopScreenShare.Uid1 && userAccount != _desktopScreenShare.Uid2)
            {
                ScreenShareWhileVideoCallS.MakeVideoView(userAccount, _desktopScreenShare.GetChannelName(), VIDEO_SOURCE_TYPE.VIDEO_SOURCE_REMOTE);
            }
        }

        public override void OnUserOffline(RtcConnectionS connection, string userAccount, USER_OFFLINE_REASON_TYPE reason)
        {
            _desktopScreenShare.Log.UpdateLog(string.Format("OnUserOffLine uid: ${0}, reason: ${1}", userAccount,
                (int)reason));
            if (userAccount != _desktopScreenShare.Uid1 && userAccount != _desktopScreenShare.Uid2)
            {
                ScreenShareWhileVideoCallS.DestroyVideoView(userAccount);
            }
        }
    }
