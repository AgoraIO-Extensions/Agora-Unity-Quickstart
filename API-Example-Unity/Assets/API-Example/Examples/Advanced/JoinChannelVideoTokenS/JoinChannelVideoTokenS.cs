...
namespace Agora_RTC_Plugin.API_Example.Examples.Advanced.JoinChannelVideoTokenS
{
    public class JoinChannelVideoTokenS : MonoBehaviour
    {
        ...
        {
            JoinChannelVideoTokenS._channelToken = newToken;
        ...
        {
            RtcEngine.RenewToken(JoinChannelVideoTokenS._channelToken);
        }
        ...
        private void JoinChannel()
        {
            ...
            RtcEngine.JoinChannel(_channelToken, _channelName, "", "123");
        }
    ...
    {
        private readonly JoinChannelVideoTokenS _helloVideoTokenAgora;

        internal UserEventHandlerS(JoinChannelVideoTokenS helloVideoTokenAgora)
        {
        ...
        public override void OnJoinChannelSuccess(RtcConnectionS connection, int elapsed)
        {
            ...
            _helloVideoTokenAgora.Log.UpdateLog(string.Format("New Token: {0}",
                JoinChannelVideoTokenS._channelToken));
            // HelperClass.FetchToken(tokenBase, channelName, 0, this.RenewOrJoinToken);
            JoinChannelVideoTokenS.MakeVideoView("");
        }
        ...
        public override void OnLeaveChannel(RtcConnectionS connection, RtcStats stats)
        {
            ...
            JoinChannelVideoTokenS.DestroyVideoView("");
        }
        ...
        {
            _helloVideoTokenAgora.Log.UpdateLog(string.Format("OnUserJoined userAccount: ${0} elapsed: ${1}", userAccount,
                elapsed));
            JoinChannelVideoTokenS.MakeVideoView(userAccount, _helloVideoTokenAgora.GetChannelName());
        }

        public override void OnUserOffline(RtcConnectionS connection, string userAccount, USER_OFFLINE_REASON_TYPE reason)
        {
            _helloVideoTokenAgora.Log.UpdateLog(string.Format("OnUserOffLine userAccount: ${0}, reason: ${1}", userAccount,
                (int)reason));
            JoinChannelVideoTokenS.DestroyVideoView(userAccount);
        }

        public override void OnTokenPrivilegeWillExpire(RtcConnectionS connection, string token)
        {
            _helloVideoTokenAgora.StartCoroutine(HelperClass.FetchToken(JoinChannelVideoTokenS._tokenBase,
                _helloVideoTokenAgora.GetChannelName(), 0, _helloVideoTokenAgora.RenewOrJoinToken));
        }

        public override void OnConnectionStateChanged(RtcConnectionS connection, CONNECTION_STATE_TYPE state,
