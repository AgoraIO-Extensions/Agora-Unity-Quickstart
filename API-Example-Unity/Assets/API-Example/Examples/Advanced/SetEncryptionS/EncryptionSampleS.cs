...
namespace Agora_RTC_Plugin.API_Example.Examples.Advanced.SetEncryptionS
{
    public class EncryptionSampleS : MonoBehaviour
    {
    ...
    {
        private readonly EncryptionSampleS _encryptionSample;

        internal UserEventHandlerS(EncryptionSampleS encryptionSample)
        {
        ...
        public override void OnJoinChannelSuccess(RtcConnectionS connection, int elapsed)
        {
            ...
            EncryptionSampleS.MakeVideoView("");
        }
        ...
        {
            _encryptionSample.Log.UpdateLog(string.Format("OnUserJoined userAccount: ${0} elapsed: ${1}", userAccount, elapsed));
            EncryptionSampleS.MakeVideoView(userAccount, _encryptionSample.GetChannelName());
        }

        public override void OnUserOffline(RtcConnectionS connection, string userAccount, USER_OFFLINE_REASON_TYPE reason)
        {
            _encryptionSample.Log.UpdateLog(string.Format("OnUserOffLine userAccount: ${0}, reason: ${1}", userAccount,
                (int)reason));
            EncryptionSampleS.DestroyVideoView(userAccount);
        }
        ...
        public override void OnEncryptionError(RtcConnectionS connection, ENCRYPTION_ERROR_TYPE errorType)
        {
