...
namespace Agora_RTC_Plugin.API_Example.Examples.Advanced.VoiceChangerS
{
    public class VoiceChangerS : MonoBehaviour
    {
    ...
    {
        private readonly VoiceChangerS _voiceChanger;

        internal UserEventHandlerS(VoiceChangerS voiceChanger)
        {
        ...
        {
            _voiceChanger.Log.UpdateLog(string.Format("OnUserJoined uid: ${0} elapsed: ${1}", userAccount,
                elapsed));
        }

        public override void OnUserOffline(RtcConnectionS connection, string userAccount, USER_OFFLINE_REASON_TYPE reason)
        {
            _voiceChanger.Log.UpdateLog(string.Format("OnUserOffLine uid: ${0}, reason: ${1}", userAccount,
                (int)reason));
        }
