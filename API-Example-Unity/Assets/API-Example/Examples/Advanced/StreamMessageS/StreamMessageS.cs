...
namespace Agora_RTC_Plugin.API_Example.Examples.Advanced.StreamMessageS
{
    ...
    {
        private readonly StreamMessageS _streamMessage;

        internal UserEventHandlerS(StreamMessageS videoSample)
        {
        ...
        {
            _streamMessage.Log.UpdateLog(string.Format("OnUserJoined uid: ${0} elapsed: ${1}", userAccount, elapsed));

        }

        public override void OnUserOffline(RtcConnectionS connection, string userAccount, USER_OFFLINE_REASON_TYPE reason)
        {
            _streamMessage.Log.UpdateLog(string.Format("OnUserOffLine uid: ${0}, reason: ${1}", userAccount,
                (int)reason));

        }

        public override void OnStreamMessage(RtcConnectionS connection, string remoteUserAccount, int streamId, byte[] data, ulong length, ulong sentTs)
        {
            string streamMessage = System.Text.Encoding.Default.GetString(data);
            _streamMessage.Log.UpdateLog(string.Format("OnStreamMessage remoteUid: {0}, stream message: {1}", remoteUserAccount, streamMessage));
        }
        ...
        {
            _streamMessage.Log.UpdateLog(string.Format("OnStreamMessageError remoteUid: {0}, streamId: {1}, code: {2}, missed: {3}, cached: {4}", remoteUserAccount, streamId, code, missed, cached));
        }
    }
