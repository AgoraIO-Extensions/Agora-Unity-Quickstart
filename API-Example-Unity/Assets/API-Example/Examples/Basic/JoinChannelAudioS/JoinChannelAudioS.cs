...
namespace Agora_RTC_Plugin.API_Example.Examples.Basic.JoinChannelAudioS
{
    public class JoinChannelAudioS : MonoBehaviour
    {
    ...
    {
        private readonly JoinChannelAudioS _audioSample;

        internal UserEventHandlerS(JoinChannelAudioS audioSample)
        {
        ...
        {
            _audioSample.Log.UpdateLog(string.Format("OnUserJoined uid: ${0} elapsed: ${1}", userAccount, elapsed));
        }

        public override void OnUserOffline(RtcConnectionS connection, string userAccount, USER_OFFLINE_REASON_TYPE reason)
        {
            _audioSample.Log.UpdateLog(string.Format("OnUserOffLine uid: ${0}, reason: ${1}", userAccount,
                (int)reason));
        }
    }
